...
info = StreamInfo('DrumTrackMarkerStream', 'Markers', 1, 0, 'string', 'myuidw43536')

# next make an outlet
outlet = StreamOutlet(info)

print("sending drum track markers...")
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='E:\\studies\\college\\IIT Gandhinagar\\SEM III\\Internship\\Experimental Design\\Drumming-EEG-Expt-Psychopy\\test-real-time_lastrun.py',
...
# Initialize components for Routine "PleasantnessRating"
PleasantnessRatingClock = core.Clock()
KeyboardResponse = visual.TextStim(win=win, name='KeyboardResponse',
    text='Please enter the pleasantness \nrating of the song from 0 to 9.',
    font='Open Sans',
    pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
    languageStyle='LTR',
...
text = visual.TextStim(win=win, name='text',
    text='Please take a 1 minute break.\nPress SPACE after the break to continue.\n',
    font='Open Sans',
    pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
    languageStyle='LTR',
...
for thisTrial in trials:
    ...
    outlet.push_sample(["FixBeg-" + TrackName])
    # keep track of which components have finished
    ...
    outlet.push_sample(["FixEnd-" + TrackName])
    ...
    outlet.push_sample(["AudBeg-" + TrackName])
    # keep track of which components have finished
    ...
    outlet.push_sample(["AudEnd-" + TrackName])
    
    # ------Prepare to start Routine "PleasantnessRating"-------
    continueRoutine = True
    # update component parameters for each repeat
    key_resp_3.keys = []
    key_resp_3.rt = []
    _key_resp_3_allKeys = []
    # keep track of which components have finished
    PleasantnessRatingComponents = [KeyboardResponse, key_resp_3]
    for thisComponent in PleasantnessRatingComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    t = 0
    _timeToFirstFrame = win.getFutureFlipTime(clock="now")
    PleasantnessRatingClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
    frameN = -1
    
    # -------Run Routine "PleasantnessRating"-------
    while continueRoutine:
        # get current time
        t = PleasantnessRatingClock.getTime()
        tThisFlip = win.getFutureFlipTime(clock=PleasantnessRatingClock)
        tThisFlipGlobal = win.getFutureFlipTime(clock=None)
        frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
        # update/draw components on each frame
    ...
    # -------Ending Routine "PleasantnessRating"-------
    for thisComponent in PleasantnessRatingComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    ...
    if theseKeys != []:
        for thisKey in theseKeys:
            outlet.push_sample(["Rating-"+ thisKey.name +"-"+ TrackName])
    # the Routine "PleasantnessRating" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    ...
    if trials.thisN == 13:
        continueRoutine = True
        outlet.push_sample(["BreakBeg"])
    else:
        continueRoutine = False
    # keep track of which components have finished
    BreakComponents = [text, break_end]
    ...
    while continueRoutine:
        ...
        # *break_end* updates
        waitOnFlip = False
        if break_end.status == NOT_STARTED and tThisFlip >= 60-frameTolerance:
            # keep track of start time/frame for later
            ...
            win.callOnFlip(break_end.clearEvents, eventType='keyboard')  # clear events on next screen flip
    ...
    if break_end.keys in ['', [], None]:  # No response was made
        break_end.keys = None
    trials.addData('break_end.keys',break_end.keys)
    if break_end.keys != None:  # we had a response
        trials.addData('break_end.rt', break_end.rt)
    trials.addData('break_end.started', break_end.tStartRefresh)
    trials.addData('break_end.stopped', break_end.tStopRefresh)
